...
function getKeyHighlights(description: string): string[] {
  ...
  // Split on periods that are NOT between digits (preserves 99.94%, 99.98%, etc.)
  const sentences = description.split(/(?<!\d)[.!?]+(?!\d)/).filter(s => s.trim().length > 0);

  sentences.forEach(sentence => {
    const trimmed = sentence.trim();
    if (trimmed.length > 15) {
      const hasMetrics = /\d+%?|\d+[Kk]|\d+[Mm]|\d+ lakh|\d+ million/.test(trimmed);
      const hasAchievement = /increased|reduced|improved|boosted|achieved|delivered|maintained|led|developed|implemented|created|optimized|built|launched|shipped|owned|redesigned|scaled|architected|mentored|established|enabling|achieving/i.test(trimmed);
...
function getTechnologies(description: string): string[] {
  const techKeywords = [
    'Swift', 'SwiftUI', 'Objective-C', 'UIKit', 'Core Data', 'SwiftData',
    'Combine', 'Swift Concurrency', 'async/await', 'Actors', 'GCD',
    'AVFoundation', 'MapKit', 'CoreLocation', 'ARKit', 'Vision',
    'Core ML', 'URLSession', 'WebSockets', 'SSE',
    'MVVM', 'Clean Architecture', 'Modular', 'Protocol-Oriented',
    'Dependency Injection', 'XCTest', 'XCUITest',
    'SDK', 'SaaS', 'Enterprise', 'Payments', 'Apple Pay', 'Razorpay', 'Stripe',
    'Firebase', 'CI/CD', 'Fastlane', 'GitHub Actions',
    'TestFlight', 'App Store Connect', 'CocoaPods', 'SPM',
